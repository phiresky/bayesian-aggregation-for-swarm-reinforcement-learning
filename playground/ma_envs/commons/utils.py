...
def get_distance_matrix(points, world_size=None, torus=False, add_to_diagonal=0):
    distance_matrix = np.empty((len(points), len(points)), dtype="float32")
    for i, p in enumerate(points):
        distance_matrix[i] = get_distances(
            points, p, torus=torus, world_size=world_size
        )
    distance_matrix = distance_matrix + np.diag(
        add_to_diagonal * np.ones(points.shape[0])
    )
...
def get_euclid_distances(points, matrix=True):
    if matrix:
        dist = ssp.distance.squareform(ssp.distance.pdist(points, "euclidean"))
    else:
...
def get_adjacency_matrix(distance_matrix, max_dist):
    return np.array((distance_matrix < max_dist) & (distance_matrix > 0.0), dtype=float)
...
def get_basis_fct_2d(radius, nr_dist_basis_fct, nr_bear_basis_fct, scale=(0.05, 0.05)):
    ...
    out = np.stack(basis_fct, axis=0).reshape(
        nr_dist_basis_fct * nr_bear_basis_fct, y.size, x.size
    )
    return out
