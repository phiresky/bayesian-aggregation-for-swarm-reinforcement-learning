...
class Evader(Agent):
    def __init__(self, experiment):
        ...
        if self.torus:
            self.bounding_box = np.array(
                [0.0, 2 * self.world_size, 0.0, 2 * self.world_size]
            )
        else:
            self.bounding_box = np.array([0.0, self.world_size, 0.0, self.world_size])
    ...
    def step(self, agent, world):
        if self.torus:
            ...
            nodes_left[:, 0] = self.bounding_box[0] - (
                nodes_left[:, 0] - self.bounding_box[0]
            )
            nodes_right = np.copy(nodes_center_sub)
            nodes_right[:, 0] = self.bounding_box[1] + (
                self.bounding_box[1] - nodes_right[:, 0]
            )
            nodes_down = np.copy(nodes_center_sub)
            nodes_down[:, 1] = self.bounding_box[2] - (
                nodes_down[:, 1] - self.bounding_box[2]
            )
            nodes_up = np.copy(nodes_center_sub)
            nodes_up[:, 1] = self.bounding_box[3] + (
                self.bounding_box[3] - nodes_up[:, 1]
            )
        ...
        for i, ridge in enumerate(vor.ridge_points):
            if evader_sub in set(ridge) and np.all([r <= evader_sub for r in ridge]):
                ...
                if neighbor in closest_pursuer:
                    ...
                    return (alpha_h_i * eta_h_i - alpha_v_i * eta_v_i) / np.sqrt(
                        alpha_h_i ** 2 + alpha_v_i ** 2
                    )
